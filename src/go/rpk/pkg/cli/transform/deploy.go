...
`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if cfg.OutputTopics == nil || len(cfg.OutputTopics) == 0 {
	...
	cmd.Flags().StringSliceVarP(&fc.outputTopics, "output-topic", "o", []string{}, "The output topic to write the transform results to")
...
func mergeProjectConfigs(lhs project.Config, rhs project.Config) (out project.Config) {
	...
	if rhs.OutputTopics != nil && len(rhs.OutputTopics) > 0 {
...
func isEmptyProjectConfig(cfg project.Config) bool {
	return cfg.Name == "" && cfg.InputTopic == "" && (cfg.OutputTopics == nil || len(cfg.OutputTopics) == 0) && (cfg.Env == nil || len(cfg.Env) == 0)
}
