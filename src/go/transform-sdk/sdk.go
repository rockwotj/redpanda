...
// ProcessWrittenRecords calls the callback for every record that is written to the input topic.
//
// ProcessWrittenRecords should be called in a package's `main` function to register the transform function that will be applied.
func ProcessWrittenRecords(fn OnRecordWrittenCallback) {
	process(fn)
}
