...
mod bridge {
    ...
    unsafe extern "C++" {
        ...
        type HostContext;
        fn call_host_fn(module: &str, name: &str, params: &[u64], result: &[u64]) -> UniquePtr<AsyncResult>;
    }
}
...
impl Engine {
    fn create_linker(&mut self) -> Box<Linker> {
        Box::new(Linker { linker: wasmtime::Linker::new(&self.engine)})
    }
...
impl Linker {
    fn add_host_fn(&mut self, module: &str, name: &str, params: &[wasmtime::ValType], results: &[wasmtime::ValType]) -> Result<()> {
        self.linker.func_new_async(
            module,
            name,
            wasmtime::FuncType::new(params, results), 
            |caller, params, results| caller.data_mut()
        )?
    }
}
