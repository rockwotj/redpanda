...
public:
    wasmtime_engine_v2(
      ...
      ss::logger* logger)
      : wasmtime_engine(
        engine, std::move(metadata), std::move(preinitialized), sr, logger) {}
...
private:
    static bool is_v2_abi(wasmtime_module_t* user_module) {
        wasm_importtype_vec_t imports;
        wasm_importtype_vec_new_empty(&imports);
        auto _ = ss::defer(
          [&imports]() { wasm_importtype_vec_delete(&imports); });
