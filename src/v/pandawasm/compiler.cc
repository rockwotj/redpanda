...
jit_compiler::jit_compiler()
  : _logger(std::make_unique<seastar_log_adapter>()) {
    _code.init(_rt.environment(), _rt.cpuFeatures());
    expression_in_debug_mode(_code.setLogger(_logger.get()));
}

jit_compiler::~jit_compiler() = default;

std::unique_ptr<jit_function_compiler>
jit_compiler::add_func(function::metadata meta) {
    return std::make_unique<jit_function_compiler>(&_code, std::move(meta));
}

// A useful cheatsheet:
// https://cs.brown.edu/courses/cs033/docs/guides/x64_cheatsheet.pdf
jit_function_compiler::jit_function_compiler(
  CodeHolder* ch, function::metadata meta)
  : _asm(ch)
...
void jit_function_compiler::prologue() {
    ...
    int32_t offset = 0;
    for (size_t i = 0; i < _func_meta.signature.result_types.size(); ++i) {
        TypeId vt = valtype_to_id(_func_meta.signature.result_types[i]);
    ...
    //_frame.setLocalStackSize(_func_meta.max_stack_size_bytes + offset);
    _frame.finalize();
...
void jit_function_compiler::operator()(op::get_local_i32 op) {
    ...
    auto reg = allocate_register(&loc);
    int32_t offset = static_cast<int32_t>(op.idx) * CHAR_BIT;
    _asm.mov(reg.r32(), x86::Mem(x86::rsp, offset));
    _stack->push(std::move(loc));
}
