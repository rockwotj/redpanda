...
constexpr static size_t bits_per_key = 10; // 10 bits per key
// 0.69 ~ ln(2)
constexpr static uint8_t num_probes = static_cast<uint8_t>(bits_per_key * 0.69);
constexpr static uint8_t max_probes = 30;
constexpr static size_t max_allocation_size = 128_KiB;

void create(const chunked_vector<ss::sstring>& keys, iobuf* filter) {
...
void filter_builder::generate_filter() {
    ...
    bloom_filter::create(_keys, &_filter);
    _keys.clear();
}
