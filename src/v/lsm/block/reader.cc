...
public:
    ...
    ss::future<> seek_to_last() final {
        ...
        while (parse_next_key() && next_entry_offset() < _restarts) {
            // Keep skipping
        }
        return ss::now();
    }
...
private:
    ...
    uint32_t get_restart_point(uint32_t index) {
        return _data->read_fixed32(_restarts + (index * sizeof(uint32_t)));
    }
    ...
    uint32_t _restarts;     // Offset of restart array (list of fixed32)
