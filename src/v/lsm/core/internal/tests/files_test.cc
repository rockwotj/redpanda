...
TEST(Files, ManifestFileName) {
    ...
    EXPECT_EQ(
      "00000000000000000123.manifest", manifest_file_name(123_file_id));
...
TEST(Files, ParseFilename_SstFile) {
    ...
    EXPECT_THAT(
      parse_filename(sst_file_name(file_id{std::numeric_limits<uint64_t>::max()})),
      Optional(AllOf(
        Field(&parsed_filename::type, Eq(file_type::sst)),
        Field(&parsed_filename::id, Eq(file_id{std::numeric_limits<uint64_t>::max()})))));
}
...
TEST(Files, ParseFilename_ManifestFile) {
    ...
    EXPECT_THAT(
      parse_filename(manifest_file_name(file_id{std::numeric_limits<uint64_t>::max()})),
      Optional(AllOf(
        Field(&parsed_filename::type, Eq(file_type::manifest)),
        Field(&parsed_filename::id, Eq(file_id{std::numeric_limits<uint64_t>::max()})))));
}
