...
ss::future<> multipart_upload::put(iobuf data) {
    if (_finalized) {
        co_await ss::make_exception_future<>(std::logic_error(
          fmt::format("Cannot put() on finalized multipart upload")));
    }
...
ss::future<> multipart_upload::complete() {
    if (_finalized) {
        co_await ss::make_exception_future<>(
          std::logic_error(fmt::format("Multipart upload already finalized")));
    }
    ...
    try {
        ...
        if (_buffer.size_bytes() > 0) {
            vlog(
              ...
              _part_number,
              _buffer.size_bytes());
            co_await _state->upload_part(_part_number++, std::move(_buffer));
        }
        ...
        vlog(
          ...
          _part_number - 1);
        co_await _state->complete_multipart_upload();
    } catch (...) {
        // Attempt abort on completion failure
        auto ex = std::current_exception();
        vlog(
          s3_log.error,
          "Multipart upload completion failed, attempting abort: {}",
          ex);
