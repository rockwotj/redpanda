...
/*
 * The kvstore manager runs on each shard and is responsible for dispatching
...
public:
    ...
    // manager.
    //
    // These callbacks are invoked until the manager is stopped.
    void on_partition_leader(notification_cb_t) noexcept;

    // Start the manager. After this is invoked, all notifications
    ...
    // Stop the manager. After this point notifications will no longer be
    // invoked.
    ss::future<> stop();
