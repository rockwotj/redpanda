...
public:
    ...
    using unmanage_cb_t
      = ss::noncopyable_function<void(model::topic_partition)>;

    /// \brief Copies table with ntps matching a given topic namespace
    ntp_table_container
    get_topic_partition_table(const model::topic_namespace& tp_ns) const {
        return get_topic_partition_table(model::topic_namespace_view(tp_ns));
    }
