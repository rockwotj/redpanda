...
ss::future<ss::chunked_fifo<transformed_partition_data_result>>
local_service::produce(
  ...
  model::timeout_clock::duration timeout) {
    ...
    co_await ss::parallel_for_each(
      ...
      [this, timeout, &results](transformed_topic_data data) {
...
local_service::produce(
  transformed_topic_data data, model::timeout_clock::duration timeout) {
    const auto& topic = data.topic;
    ss::chunked_fifo<transformed_partition_data_result> results;
    co_await ss::parallel_for_each(
      std::make_move_iterator(data.partitions.begin()),
      std::make_move_iterator(data.partitions.end()),
      [this, timeout, &topic, &results](transformed_partition_data data) {
          return produce(topic, std::move(data), timeout)
            .then([&results](auto r) { results.push_back(std::move(r)); });
      });
    co_return results;
}

ss::future<transformed_partition_data_result> local_service::produce(
  model::topic_view topic,
  transformed_partition_data data,
  model::timeout_clock::duration timeout) {
    auto ktp = model::ktp(topic, data.partition_id);
    auto shard = _shard_table->local().shard_for(ktp);
    if (!shard) {
        co_return transformed_partition_data_result(
          ktp.to_tp(), cluster::errc::not_leader);
    }

    auto topic_cfg = _metadata_cache->local().get_topic_cfg(ktp.as_tn_view());
    if (!topic_cfg) {
        co_return transformed_partition_data_result(
          ktp.to_tp(), cluster::errc::topic_not_exists);
    }
    ...
    for (const auto& batch : data.batches) {
        if (uint32_t(batch.size_bytes()) > max_batch_size) [[unlikely]] {
            co_return transformed_partition_data_result(
              ktp.to_tp(), cluster::errc::invalid_request);
        }
    }
    ...
    co_return transformed_partition_data_result(ktp.to_tp(), ec);
}
